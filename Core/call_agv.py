...
def check_wait_point(start, end, port_number, node_list, vehicle_list, wait_list, path_linked_list):
    ...
    for idx in range(start, end, -1):
        ...
        if wait_list[idx].using and wait_list[idx].using.status == 81:
            ...
            vehicle_list[wait_list[idx].using-1].command(a_star_path, 21)
            break
                ...
                for unload_port in unloadable_port_list:
                    ...
                    if unload_port in load_port.UNLOAD_PORT:
                        ...
                        if 340 <= load_port.NUM <= 363:
                            check_wait_point(9, -1, load_port.NUM, node_list,  vehicle_list, wait_list, path_linked_list)
                        # # 좌측 하단 3번 포트
                        elif 392 <= load_port.NUM <= 415:
                            check_wait_point(29, 19, load_port.NUM, node_list, vehicle_list, wait_list, path_linked_list)
                        # # 우측 상단 3번 포트
                        elif 364 <= load_port.NUM <= 387:
                            check_wait_point(19, 9, load_port.NUM, node_list,  vehicle_list, wait_list, path_linked_list)
                        # # 우측 하단 3번 포트
                        elif 416 <= load_port.NUM <= 439:
                            check_wait_point(39, 29, load_port.NUM, node_list, vehicle_list, wait_list, path_linked_list)
                        # # 중앙 상단 1번 포트
                            ...
                            #  break가 안 걸렸다면 충전/대기 장소도 체크하기
                                ...
                                check_wait_point(49, 41, load_port.NUM, node_list, vehicle_list, wait_list, path_linked_list)
                        # # 중앙 하단 1번 포트
                        elif 440 <= load_port.NUM <= 443:
                            check_wait_point(51, 41, load_port.NUM, node_list, vehicle_list, wait_list, path_linked_list)
