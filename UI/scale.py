...
class OperationData(QDialog):
    ...
    def initUI(self):
        ...
        self.label_capa.move(self.width() * 0.1, self.height() * 0.3)

        self.edit_capa = QLineEdit(self)
        self.edit_capa.move(self.width() * 0.4, self.height() * 0.3)
        self.edit_capa.textChanged.connect(lambda: self.validate(self.edit_capa))

        self.label_speed = QLabel("시뮬레이션 속도", self)
        self.label_speed.move(self.width() * 0.1, self.height() * 0.5)

        self.edit_speed = QLineEdit(self)
        self.edit_speed.move(self.width() * 0.4, self.height() * 0.5)
