...
class Environment(object):
    ...
    def __init__(self, vehicle_list, node_list, path_linked_list):
        self.agents = vehicle_list
        self.node_list = node_list
        self.path_linked_list = path_linked_list
        self.agent_dict = {}
    ...
    def get_neighbors(self, state):
        ...
        if self.state_valid(n):
            neighbors.append(n)
        # Move
        for next_node in self.path_linked_list[state.node]:
            node_pos = [node for node in self.node_list if node.NUM == next_node[0]][0]
            n = State(state.time + 1, (node_pos.X, node_pos.Y), next_node[0])
